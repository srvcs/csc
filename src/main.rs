use srvcs_csc::{api::Deps, config::Config, health, router, telemetry};
...
async fn main() {
    ...
    tracing::info!(addr = %cfg.bind_addr, env = %cfg.environment, "srvcs-csc listening");
    let deps = Deps {
        sin_url: cfg.sin_url.clone(),
        floatdivide_url: cfg.floatdivide_url.clone(),
    };
