...
pub const SERVICE: &str = "srvcs-csc";
pub const CONCERN: &str = "trigonometry: cosecant";
pub const DEPENDS_ON: &[&str] = &["srvcs-sin", "srvcs-floatdivide"];
...
pub struct EvalRequest {
    /// The angle in radians to take the cosecant of.
...
/// dependency's numeric `result` (on `200`) or an early-return `Response` the
...
/// `POST /` — compute `csc(value)` by composing two float primitives.
///
/// This service owns the *control flow* but delegates every arithmetic step to
/// its dependencies, exactly as specified:
///
/// 1. ask `srvcs-sin` for `s = sin(value)`;
/// 2. ask `srvcs-floatdivide` for `result = 1 / s`.
///
...
/// raise is forwarded verbatim. If a dependency is unreachable it reports itself
/// degraded (`503`).
#[utoipa::path(
    post,
    path = "/",
...
pub async fn evaluate(State(deps): State<Deps>, Json(req): Json<EvalRequest>) -> Response {
    ...
    // 1. s = sin(value)
    let s = match ask(&deps.sin_url, &json!({ "value": value }), "srvcs-sin").await {
        Ok(s) => s,
...
#[openapi(
    ...
    components(schemas(Info, EvalRequest, CscResponse))
)]
pub struct ApiDoc;
...
mod tests {
    ...
    async fn index_reports_all_dependencies() {
        ...
        assert_eq!(info.service, "srvcs-csc");
        assert_eq!(info.concern, "trigonometry: cosecant");
        assert_eq!(info.depends_on, vec!["srvcs-sin", "srvcs-floatdivide"]);
    }
}
